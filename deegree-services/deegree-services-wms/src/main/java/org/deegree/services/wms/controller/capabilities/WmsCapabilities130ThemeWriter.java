...
class WmsCapabilities130ThemeWriter {
    ...
    void writeTheme( final XMLStreamWriter writer, final Theme theme )
        ...
        writeCascaded( writer, theme.getMetadata().getCascaded() );
        ...
        writeStyles( writer, theme.getMetadata() );
    ...
    private void writeQueryable( final XMLStreamWriter writer, final Theme theme )
                            throws XMLStreamException {
        if ( theme.getMetadata().isQueryable() && theme.getMetadata().getName() != null ) {
    ...
    private void writeName( final XMLStreamWriter writer, final Theme theme )
        ...
        if ( theme.getMetadata().getName() != null ) {
            writeElement( writer, WMSNS, "Name", theme.getMetadata().getName() );
        }
    }
    ...
    private void writeKeywordList( final XMLStreamWriter writer,
        ...
        if ( keywordList != null && !keywordList.isEmpty() ) {
            ...
            for ( final Pair<List<LanguageString>, CodeType> keywords : keywordList ) {
                for ( final LanguageString keyword : keywords.first ) {
                    writeElement( writer, WMSNS, "Keyword", keyword.getString() );
                }
            }
            writer.writeEndElement();
        }
    ...
    private void writeScaleDenominators( final XMLStreamWriter writer, final Theme theme )
        ...
        if ( theme.getMetadata() != null && theme.getMetadata().getScaleDenominators() != null ) {
            final DoublePair themeScales = theme.getMetadata().getScaleDenominators();
