...
public class OracleDDLCreator extends DDLCreator {
    ...
    private int triggerNo = 1;

    private int spatialIdxNo = 1;

    private int pkConstraintNo = 1;
    ...
    private List<StringBuffer> getGeometryCreate( GeometryMapping mapping, DBField dbField, TableName table ) {
        ...
        String idxName = "SPATIAL_IDX_" + ( spatialIdxNo++ );
        sql.append( "CREATE INDEX " + idxName + " ON " + table.toString().toUpperCase() + "(" + column.toUpperCase()
    ...
    protected StringBuffer createJoinedTable( TableName fromTable, TableJoin jc, List<StringBuffer> ddls ) {
        ...
        sb.append( " (\n    " );
        String constraintName = "pk_constraint_" + pkConstraintNo++;
        ...
        ddls.add( sb );
        String sequenceName = "id_seq_" + ( seqNo++ );
        ddls.add( new StringBuffer( "create sequence " ).append( sequenceName ).append( " start with 1 increment by 1 nomaxvalue" ) );
        String triggerName = "id_trigger_" + ( triggerNo++ );
        ddls.add( new StringBuffer( "create or replace trigger " ).append( triggerName ).append( " before insert on " ).append( jc.getToTable() ).append( " for each row begin select " ).append( sequenceName ).append( ".nextval into :new.id from dual; end;" ) );
        return sb;
    }
    ...
    @Override
    protected String getPkConstraintName( TableName ftTable ) {
        return "pk_constraint_" + pkConstraintNo++;
    }
}
